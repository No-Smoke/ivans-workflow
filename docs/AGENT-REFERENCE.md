...
All agent definition files (`core/agents/*.md`) must start with the framework header before the YAML frontmatter:
...
This header identifies the file as framework-managed and is checked by the audit process. Custom project agents in `.claude/agents/` do not require this header.
